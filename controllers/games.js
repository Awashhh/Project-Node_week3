...
const addGameController = async (req, res) => {
		...
		if (req.isNew) {
			...
			res.send({ status: 'error', message: 'Игра с таким именем уже есть.' }).status(400)
			return
		}
...
const deleteGames = async (req, res) => {
		...
		req.game = req.games.find(game => game.id === id)
...
module.exports = { sendAllGames, deleteGames, addGameController }